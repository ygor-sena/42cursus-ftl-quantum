...
if [ $# -eq 0 ]; then
  echo "Usage: $0 <container_or_id_name>"
  exit 1
fi

# Retrieve the Jupyter URL from Docker logs
JUPYTER_URL=$(docker logs $CONTAINER_NAME 2>&1 | grep -oP 'https?://\S+/tree\?token=\S+' | tail -n 1)

# Check if the URL was found in the logs
if [ -z "$JUPYTER_URL" ]; then
  echo "Error: Jupyter URL not found in logs for container $CONTAINER_NAME"
  exit 1
fi
